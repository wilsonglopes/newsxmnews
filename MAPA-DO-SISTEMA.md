...
| 22 | `extrairImagemDoConteudo` sem condicional | RSS da CNN inclui seção "Leia mais" com thumbnails de outras matérias; função pegava o primeiro `<img>` do body HTML de TODAS as fontes → imagem errada (de artigo relacionado, não da matéria) | `extrairImagemDoConteudo` só é chamado quando `source.extract_body_image === true`; fontes com `false` ficam com `image_url = null` e `fetchFullContent` busca `og:image` correto |
| 23 | Instagram code 9007 — container não pronto | `media_publish` chamado imediatamente após criar o container; Instagram ainda processava a imagem → `Media ID is not available` | Polling de `status_code` entre etapa 1 e etapa 2: `GET /{creation_id}?fields=status_code` até `FINISHED` (máx 30s, a cada 3s) antes de chamar `media_publish` |
